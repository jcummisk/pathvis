...
(function(document) {
	...
	function inspectorMouseOver(e) {
		...
		// Set outline:
		element.style.outline = '2px solid #8900ff';
...
function inspectorAddExp(e){
    var x = document.getElementById("showVisPop");
    x.style.display = "block";
    var holdContent = cssPath(e.target);
    var c = document.getElementById('updateInput');
    $(c).val(holdContent).select()
document.getElementById('object');
	...
	function inspectorCancel(e) {
        ...
        if (e == null && event.keyCode === 27) { // IE (won't work yet):
			...
			document.detachEvent("click", inspectorAddExp);
			...
			document.removeEventListener("click", inspectorAddExp, true);
	...
	if ( document.addEventListener ) {
		...
		document.attachEvent("click", inspectorAddExp);
